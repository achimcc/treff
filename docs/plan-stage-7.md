...
| | |
|---|---|
| Open | **Task 1** — `likes`: table, toggle, counts, the entry in the bell |
| Open | **Task 2** — `POST /p/{id}/like`, the heart on the pages, `like.js` |
| Open | **Task 3** — the bell says it: page, JSON, `bell.js` |
| Open | **Task 4** — the face of the forum: recent, sections, counts, numbers |
| Open | **Task 5** — preview by eye, 0.9.0 |
...
- [ ] `db::inbox`: `Entry::Likes { topic_id, topic_title, post_id, count, latest_name, at, unread }`;
...
- [ ] Unit tests (`src/db/likes.rs`, `src/db/inbox.rs`): toggle on, toggle
...
- [ ] `POST /p/{id}/like` → `like_post`: `may_read` or 403; `toggle`;
...
- [ ] `GET /assets/like.js` like the other two (`script()` helper, its own
  ETag); the layout loads it with `defer`; `tests/frame.rs` counts three
  `<script` now.
- [ ] Views: `TopicView` gets `likes: &HashMap<i64, likes::Summary>`;
...
- [ ] i18n: `like = "Like"/"Gefaellt mir"`, `unlike = "Unlike"/"Gefaellt mir nicht mehr"`,
...
- [ ] `like.js`: for every `form.like`, on `submit` → `preventDefault`,
...
- [ ] `style.css`: `.like` (inline-flex, at the post's foot, mono 12px,
...
- [ ] Route tests (`tests/likes.rs`): a like shows `class="count">1<`, the
...
- [ ] `tests/js/like.html`: a page with two forms and a stubbed `fetch`
...
- [ ] Green ×4, commit: `Stage 7, task 2: the heart under every post`.
...
- [ ] `entry_json`: `Entry::Likes` → `{"kind":"likes","title","count","author": latest_name,"at","unread","link": "/t/{topic}#p{post}"}`.
- [ ] i18n: `likes_your_post_in = "likes your post in"/"gefaellt dein Beitrag in"`,
...
- [ ] `notifications_page`: the arm, link to the post, byline the moment.
  `nothing_new` mentions likes now (both languages).
- [ ] `bell.js`: the `likes` branch with the same wording from three new
...
- [ ] Route tests (`tests/inbox.rs`): a like from ben puts `1` on ada's
...
- [ ] Green ×4, commit: `Stage 7, task 3: the bell tells the author`.
...
- [ ] `db::topics`: `LastPost` is joined by a new `Counts { replies: i64, likes: i64, unread: bool }`
...
- [ ] `TopicRow` gets `counts: Counts`; `views::category_index` takes
...
- [ ] `views::section_nav(space, current: &str)` — `nav.sections` with
...
- [ ] `table.threads`: columns Topic · Replies · Likes · Last reply
  ...
  column does today.
- [ ] Topic page: `p.meta` under `h1` — section link, `N replies`
...
- [ ] Timeline entries: a foot line with the heart (task 2) and
  `N comments` (`comments_one/comments_many`) as a link to `/t/{id}`.
- [ ] Tests (`tests/face.rs`, route level): the front page lists the topic
...
- [ ] Green ×4, commit: `Stage 7, task 4: the face of the forum`.
...
- [ ] `tests/preview.rs`: a few likes (ben on ada's opener, ada and ben on a
...
- [ ] `CHANGELOG.md` 0.9.0, `README.md` status, `Cargo.toml` version,
...
- [ ] In the homeserver repository: `flake.nix` → `v0.9.0`,
